...
- **Investissements** (`investments_screen`) — l'écran unique de tout ce que l'organisation acquiert, en **deux rayons empilés dans un même scroll** (pas d'onglets : l'intérêt de la fusion est de mettre les investissements en concurrence dans le même champ de vision). Les deux servent la même **carte d'Actif**, et le nombre de colonnes leur est commun, calculé sur la largeur disponible.
  - 📦 **L'étal du sprint** — 2 candidats + 2 pratiques. Embaucher coûte des pièces et ajoute une masse salariale récurrente ; 🤝 1:1 paie en Énergie la révélation d'un trait caché avant de signer.
  - 🃏 **Les grandes décisions** — 2 cartes de `cards.json` **tirées elles aussi** (pioche "sac", carnet §21) ; l'impact est affiché en ressources (🫶 −35, 🧱 −10…) avec la note d'axe comme texte de ligne, et non plus en axes abstraits. "Activer (1 slot)" convertit la carte en deltas réels (via `EffectResolver`, avec les multiplicateurs d'époque), la marque comme Fondation active et la sort du sac pour de bon. Le compteur de slots vit dans le titre du rayon, le profil d'équipe dans l'en-tête du Panneau de bord.

  Toute l'offre est tirée **une fois par sprint** (`SprintState.get_shop_offer()`) — revenir sur l'écran ne re-tire pas. Seul le bouton 🎲 **Re-tirer l'offre** la rejoue, contre des pièces : 1 🪙, puis +1 à chaque re-tirage du sprint, remis à sa base au sprint suivant (`balance.json` → `shopDraw.reroll`). Il re-tire les deux rayons ensemble.
...
- **Catalogue de décisions trop court pour son tirage** : depuis que les grandes décisions sont tirées (carnet §21), la rareté ne se sent que si le catalogue dépasse largement ce qu'on en voit. Il n'y a que 5 cartes pour la Transformation agile et 2 sont tirées par sprint : le sac tourne trop vite. Le système vise 10-15 cartes — c'est du contenu à écrire, pas une mécanique à revoir.
