...
La boucle complète d'un sprint est jouable de bout en bout (Accueil → **choix du scénario** → **choix de l'entreprise** → Inbox → Roadmap → Investissements → Résolution → sprint suivant), avec des données réelles issues de `data/*.json`. Le mandat se joue par trimestres de 3 sprints jusqu'à une vraie fin — quota manqué, faillite, exode d'équipe, sortie réussie après T4, etc. — les cinq jauges et les deux monnaies (💥 Impact, 💰 Revenue) persistent et évoluent réellement d'un sprint à l'autre, avec des abonnements récurrents selon le modèle économique du scénario. Après T4, le joueur peut sortir ou poursuivre un mandat long aux quotas croissants. Ce n'est pas encore un jeu *équilibré* (voir `docs/carnet-de-regles.md` §14-16 pour les décisions de conception, et `data/balance.json` pour tous les nombres ajustables), mais la boucle cœur fonctionne de bout en bout.
...
- **Entreprise** (`company_select_screen`) — les entreprises de `companies.json` rattachées au scénario choisi (deux pour la Transformation agile), chacune avec sa propre accroche/description façon offre d'emploi et un profil d'équipe (junior/senior) qui sera fixé pour tout le mandat. Le choix appelle `SprintState.reset_run(era_id, company_id)` (réinitialise les jauges et les deux monnaies, fixe l'entreprise, l'équipe et le modèle économique) puis ouvre l'Inbox.
...
- **Roadmap** (`roadmap_screen`) — 4 à 5 items de `backlog.json` tirés et conservés pour le sprint, dont les epics déjà engagés. Les coûts sont visibles; ROI, impact client et risque sont masqués jusqu'à une pratique ou l'action Plonger. Les epics consomment un investissement libre en points sur plusieurs sprints et ne livrent leurs effets qu'à la complétion.
- **Investissements** (`investments_screen`) — l'écran unique de tout ce que l'organisation acquiert : **un seul rayon de 6 emplacements où les trois types se mélangent** (pas d'onglets, pas de rayons séparés — l'intérêt de la fusion est de mettre les investissements en concurrence dans le même champ de vision). Les trois servent la même **carte d'Actif** ; le nombre de colonnes est calculé sur la largeur disponible.
  ...
  - ✨ **Pratiques** — permanentes pour le mandat, +2 Cynisme à l'achat (+4 sous l'exigence « Comité de pilotage »).
...
- **Résolution** (`resolution_screen`) — applique le panier du sprint et anime le rapport `Traction × Levier = Impact`, puis la progression du quota en dernier. Tous les 3 sprints (2 sous l'exigence « Trimestre court »), le verdict compare l'Impact brut accumulé au quota : un échec mène à `remercie`, une réussite poursuit le mandat, et T4 demande explicitement de sortir ou de rester en mode long. Les objectifs qualitatifs de l'entreprise ne conditionnent plus le passage ; tous tenus, ils ajoutent 8 Budget. Les jauges, le revenu, le journal et l'alerte critique restent visibles. Un bouton dédié ouvre le plateau des Fondations.
...
- **Fin de mandat** (`mandate_end_screen`) — la fin atteinte (`endings.json`), le bilan des cinq jauges, l'époque et le profil d'équipe ; "Choisir un nouveau scénario" retourne à `scenario_screen`, "Accueil" retourne au menu.

Les 3 écrans de phase qui précèdent la Résolution (Inbox, Roadmap, Investissements) portent le **Panneau de bord** (`scenes/components/side_panel.tscn`, branché par `UIHelpers.attach_side_panel()`) : une colonne fixe à droite qui remplace l'ancienne barre de ressources horizontale. Elle affiche en continu les 6 jauges, les pièces, le bloc « Vous », le roster condensé, les actifs possédés et le quota du trimestre (Impact, sprint trimestriel, exigences actives). Les objectifs d'entreprise y sont évalués en direct comme conditions du bonus qualitatif. Un clic sur « ◂ Replier » le réduit à un rail qui conserve les signaux essentiels, dont le quota compact.
...
- **Avatars** — portraits [DiceBear](https://www.dicebear.com/) (style Notionists) générés une fois pour les 22 personnages nommés du roster et du marché. `UIHelpers.make_person_badge(nom, taille)` affiche le portrait s'il existe, sinon une pastille d'initiale — le repli intentionnel des expéditeurs collectifs et le badge d'accès des futurs candidats anonymes.
...
`SprintState` (autoload) porte l'état complet du mandat en cours : scénario, entreprise, cinq jauges, les deux monnaies (`impact_wallet`, `revenue`), panier, journal, décisions, score et état trimestriel (`quarter_index`, exigence, résultat et choix T4). L'équipe est déjà stockée dans `squads[]` (une seule squad principale aujourd'hui) ; chaque entrée porte son `roster`, backlog local, capacité, livraisons et progression d'epics. `get_roster()` fournit la vue globale aplatie pour l'UI et les règles transverses.
...
- **Portraits de futurs personnages nommés** : les 22 personnages actuels du roster et du marché ont un avatar dédié (voir `assets/THIRD_PARTY_NOTICES.md`). Tout nouveau personnage nommé devra recevoir son portrait ; les profils volontairement anonymes retombent sur une pastille d'initiale.
