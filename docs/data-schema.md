...
- `cards[]` — `id`, `costImpact` (prix d'activation en 💥 — une décision se paie comme une embauche, §21), `licensePerSeat` (la charge de 💰 Revenue prélevée chaque sprint et **par personne du roster** tant que l'outil occupe son slot — le coût d'exploitation, distinct du prix d'achat), `refId` (identifiant façon ticket), `family` (`outil-process` / `stack-technique` / `methodologie-orga`, voir §6.2), `category`, `name`, `tagline`, `effects.junior` / `effects.senior` (un objet par axe avec `value` signé et `note` explicative — le feedback immédiat sur les jauges à l'activation), `eras[]` optionnel (§15 — réserve la carte aux scénarios listés ; absent = disponible partout), `rarity` et `eraWeights` optionnels (voir *Rareté et tirage* plus bas), et `requires` optionnel.
- **Le Levier par employé (spec scoring §7.1, Lot 3).** Un outil (`family` `outil-process` ou `methodologie-orga`) peut déclarer `icon` et tout ou partie de : `perEmployee` (bonus par employé éligible), `eligibility` (condition de matching sur le roster — même grammaire que `refractory.condition`), `refractory` (`{condition, perEmployee}`), `flatModifiers[]` (`{when, value}`), `cumulative` (`{perSprint}`), `adoptionCondition` (`{condition, multiplier}` — multiplie uniquement l'additif propre à l'outil), `slotBonus` et `leverLabel` optionnel. `ScoreResolver` lit ces champs directement sur la carte ; le comportement par entreprise émerge du roster réel, sans branche par `company_id`.
- **La couche multiplicative (#37).** `leverMultiplier` s'applique **après tous les additifs** et se compose avec les autres facteurs. Forme courte : `"leverMultiplier": 1.5`. Forme qui scale : `{ "perCount": 1.6, "counter": { "kind": "active_tools", "maxCount": 3, "label": "outils actifs (3 max.)" } }` ; le facteur vaut alors `1.6 ^ nombre`, dans la limite déclarée. Les compteurs supportés sont `active_tools`, `roster_match` (`counter.condition` reprend la grammaire d'`eligibility`) et `product_tier`; `counter.offset` peut retirer une base et `counter.maxCount` borne le degré. Une `condition` optionnelle utilise la grammaire globale existante. Le même champ est accepté sur un outil, une pratique, une stratégie, `global.productTier`, un combo local ou inter-équipe. Une carte slottée doit déclarer au moins un additif (`perEmployee`/`cumulative`) ou `leverMultiplier`, sinon elle serait un piège pur (`score_resolver_cases.gd → _test_every_tool_card_has_a_lever`).
...
- `version` — version du contrat ; `careerLevels` est une table indexée par identifiant de niveau de carrière (`pm`, `lead-pm`, `director`, `cpo`, `ceo` depuis le Lot 5, mêmes ids que `careers.json` → `order`) ; `quarterQuotas` est le tableau ordonné des quotas T1 à T4. **Depuis le lot A de l'Impact-monnaie (carnet §31), ces quotas sont cumulatifs** : le board regarde le solde du portefeuille, jamais la production d'un trimestre — T2 contient donc T1, et tout ce qui a été dépensé doit être reproduit avant le verdict. Au-delà de la table (mandat long), le repli est sa **dernière ligne**, jamais une valeur écrite dans un script. Chaque niveau recalibre grossièrement sa table sur son nombre moyen de squads (`careers.json` → `squadsMin`/`squadsMax`) — un Director ne joue pas avec les chiffres d'un PM (spec §13.4). Premier jet non équilibré, comme le reste des tables de ce fichier.
...
- `qualitativeBonusImpact` — récompense en 💥 Impact, versée au portefeuille, si tous les objectifs qualitatifs de `companies.json` sont tenus en plus du quota. `qualitativeBonus` documente sa condition, son texte et ses paramètres de mode long (`enabled`, `accumulates`).
...
La 4e famille de décisions (spec scoring §7.2, Lot 3) — la seule qui n'existait pas avant ce lot. `strategies[]` — `id` (partagé avec `scoring.json` → `global.strategies`, seule table qui porte les multiplicateurs), `licenseFlat` (ce que la décision coûte en 💰 Revenue à chaque sprint, pour toujours — une stratégie s'entretient), `icon`, `name`, `tagline`, `description` (texte joueur expliquant l'effet mécanique). Ce fichier est un catalogue d'affichage, pas une table de calcul : `ScoreResolver` ne lit que `scoring.json`. Choisie au Comité de fin de trimestre (écran du lot 4, pas encore construit), 1 par trimestre, irréversible — `SprintState.choose_strategy()` / `get_strategy_options()` / `find_strategy()` sont le point d'entrée que cet écran appellera ; en attendant, l'exigence trimestrielle `board-injunction` (`quotas.json`) est la seule à en choisir une, automatiquement.
...
- `startingResources` — valeur de départ des 5 jauges ; `startingResourceOverrides` — surcharges par entreprise ; `resourceBounds` (min/max, 0-100) ; `resourceDirection` (`high-good` / `low-good`, pour savoir quel sens est "bon") ; `stateThresholds` (`goodMin`/`dangerMax`, pour l'affichage bon/attention/danger).
...
- `revenue` — 💰 le Revenue (carnet §31) : `start` et `startOverrides` par entreprise (la caisse tendue de Karavel). Sans plafond, hors de `resourceBounds`.
- `licenses` — 💰 les charges récurrentes comptées **par siège** (spec-impact-monnaie §3.6) : `defaultToolPerSeat`, `defaultPracticePerSeat`, `defaultStrategyFlat` — les défauts appliqués quand un poste ne déclare pas sa propre licence.
- `cardAxisResourceMap` — comment les 4 axes de `cards.json` se convertissent en deltas sur les 5 jauges et le Revenue (`resource`, `invert` — l'axe `financier` pointe désormais sur `revenue`).
...
Depuis #37, les rapports locaux et globaux séparent la base additive, le
produit des multiplicateurs et le Levier final. Chaque facteur devient une
ligne `lever_multiplier`, rejouée par la Résolution et reprise dans la chaîne
permanente du panneau de bord. `local.organizationCombos[]`,
`global.strategies.*`, `global.practices.*`, `global.productTier` et
`global.interSquadCombos.*` utilisent la même grammaire que `cards.json`.
